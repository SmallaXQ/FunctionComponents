...
public protocol SKUFilterDelegate: NSObjectProtocol {
    /// 属性集合个数
...
public class SKUFilter {
    ...
    /// 已选择集合 (默认顺序为选择顺序)
    public var selecteds: [IndexPath] = []
    /// 可选择集合
    public var available: Set<IndexPath> = []
    /// 最终结果 (全部选择完毕后即可获取到)
    public var result: Any?
    
    public var allAvailable: Set<IndexPath> = []
    ...
    public func selected(_ indexPath: IndexPath) {
...
extension SKUFilter {
    ...
    ///   - selecteds: 已选集合
    /// - Returns: 可选集合
    public func available(_ selected: IndexPath, with selecteds: [IndexPath]) -> Set<IndexPath> {
...
public extension SKUFilter {
    ...
    struct Condition: Hashable, Equatable {
        ...
        public static func == (lhs: SKUFilter.Condition,
